...
Parse.Cloud.job("LevelsGen", function(request, response) {
	...
	if (weekRay.indexOf(dayoweek) >= 0) {
		...
		tickquery.equalTo('LevelTrigger', true);
...
levelsgen = function(res1,res2,ticker,j,response,newres) {
	...
	if (res1.data.query.results) {
							...
							if (k == valleyray.length-1) {
								...
								for (var m = 0; m < peakray.length; m++) {
									...
									if (m == peakray.length-1) {
										...
										//var oldsupp = ticker.get('SupportVal');
										//var oldresist = ticker.get('ResistVal');

										// if (suppres && suppres.support) {
										// 	ticker.set('SupportVal', +(suppres.support));
										// 	ticker.set('LevelTrigger', false);
										// };
										// if (suppres && suppres.resist) {
										...
										if (j == newres.length-1) {
											...
											ticker.save({
												...
												error: function(error) {
													console.log('error: could not save ' + ticker.get('Ticker'));
													levelsftn(newres,j+1,response);
												}
											});
										};
		...
		if (ticker.get('Ticker')) {
			console.log('FAILD TICKER: ' + ticker.get('Ticker'))
		};
